    ...
    return [
        {
            ...
            "given_name": "John",
            "family_name": "Doe",
            "email_addresses": [{"field": "EMAIL", "email": "john.doe@example.com", "is_primary": True}],
        ...
        },
        {
            ...
            "given_name": "Jane",
            "family_name": "Smith",
            "email_addresses": [{"field": "EMAIL", "email": "jane.smith@example.com", "is_primary": True}],
...
class TestListContacts:
    ...
    @pytest.mark.asyncio
    async def test_list_contacts_basic(self, mock_context, sample_contacts, expected_formatted_contacts):
    ...
    async def test_list_contacts_with_filters(self, mock_context, sample_contacts):
        ...
        with patch('src.mcp.contact_tools.validate_filter_conditions') as mock_validate:
            with patch('src.mcp.contact_tools.optimize_filters_for_api', return_value=({}, filters)):
                with patch('src.mcp.contact_tools.apply_complex_filters', return_value=sample_contacts):
    ...
    async def test_list_contacts_with_pagination(self, mock_context, sample_contacts):
        ...
        mock_context.api_client.get_contacts.return_value = {"contacts": sample_contacts[:1]}
        mock_context.cache_manager.get.return_value = None
...
class TestSearchContactsByEmail:
    ...
    async def test_search_by_email_basic(self, mock_context, sample_contacts):
        ...
        email = "john.doe@example.com"
        mock_context.api_client.get_contacts.return_value = {"contacts": [sample_contacts[0]]}
        mock_context.cache_manager.get.return_value = None
    ...
    async def test_search_by_email_with_include(self, mock_context, sample_contacts):
        ...
        with patch('src.utils.contact_utils.process_contact_include_fields', side_effect=lambda x, y: x):
            result = await search_contacts_by_email(mock_context, email, include=include_fields)
...
class TestSearchContactsByName:
    ...
    @pytest.mark.asyncio
    async def test_search_by_name_duplicate_removal(self, mock_context, sample_contacts):
    ...
    async def test_search_by_name_with_include(self, mock_context, sample_contacts):
        ...
        with patch('src.utils.contact_utils.process_contact_include_fields', side_effect=lambda x, y: x):
            result = await search_contacts_by_name(mock_context, name, include=include_fields)
...
class TestGetContactDetails:
    ...
    @pytest.mark.asyncio
    async def test_get_contact_details_with_include(self, mock_context, sample_contacts):
        ...
        with patch('src.utils.contact_utils.process_contact_include_fields', side_effect=lambda x, y: x):
            result = await get_contact_details(mock_context, contact_id, include=include_fields)
