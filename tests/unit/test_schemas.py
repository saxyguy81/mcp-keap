...
class TestLogicalGroup:
    ...
    def test_logical_group_valid(self):
        ...
        condition2 = FilterCondition(field="email", operator="CONTAINS", value="@example")
    ...
    def test_logical_group_nested(self):
        ...
        inner_condition = FilterCondition(field="age", operator="GREATER_THAN", value=18)
        inner_group = LogicalGroup(operator=LogicalOperator.NOT, conditions=[inner_condition])
        
        outer_condition = FilterCondition(field="active", operator="EQUALS", value=True)
        outer_group = LogicalGroup(
            operator=LogicalOperator.AND,
            conditions=[outer_condition, inner_group]
        )
...
class TestContactQueryRequest:
    ...
    def test_contact_query_request_with_filters(self):
        ...
        condition = FilterCondition(field="email", operator="CONTAINS", value="@example")
        request = ContactQueryRequest(
            filters=[condition],
            limit=100,
            offset=50,
...
class TestSchemaIntegration:
    ...
    def test_complex_query_request(self):
        ...
        name_condition = FilterCondition(field="given_name", operator="EQUALS", value="John")
        email_condition = FilterCondition(field="email", operator="CONTAINS", value="@example")
        
        # Create logical group
        and_group = LogicalGroup(operator="AND", conditions=[name_condition, email_condition])
