...
def sample_contacts():
    return [
        ...
        },
        {
            ...
            "family_name": "Smith",
            "email_addresses": [
                {"field": "EMAIL", "email": "jane@example.com", "is_primary": True}
            ],
            "tag_ids": [1],
        },
    ]
...
class TestGetContactsWithTag:
    ...
    async def test_get_contacts_with_tag_basic(self, mock_context, sample_contacts):
        ...
        assert len(result) == 2
        mock_context.api_client.get_contacts_by_tag.assert_called_once_with(
            tag_id, limit=200
        )

    @pytest.mark.asyncio
    async def test_get_contacts_with_tag_with_limit(
    ...
    @pytest.mark.asyncio
    async def test_get_contacts_with_tag_with_include(
        self, mock_context, sample_contacts
    ):
        ...
        with patch(
            ...
            result = await get_contacts_with_tag(
                mock_context, tag_id, include=include_fields
            )
...
class TestCreateTag:
    ...
    async def test_create_tag_with_category(self, mock_context):
        ...
        assert result == created_tag
        mock_context.api_client.create_tag.assert_called_once_with(
            {"name": tag_name, "category": {"id": category_id}}
        )
...
class TestApplyTagsToContacts:
    ...
    async def test_apply_tags_to_contacts_success(self, mock_context):
        ...
        mock_context.api_client.apply_tags_to_contacts.assert_called_once_with(
            tag_ids, contact_ids
        )
    ...
    async def test_apply_tags_to_contacts_partial_failure(self, mock_context):
        ...
        mock_context.api_client.apply_tags_to_contacts.side_effect = ValueError(
            "Batch not supported"
        )
    ...
    async def test_apply_tags_to_contacts_exception(self, mock_context):
        ...
        mock_context.api_client.apply_tags_to_contacts.side_effect = Exception(
            "API Error"
        )
...
class TestRemoveTagsFromContacts:
    ...
    async def test_remove_tags_from_contacts_exception(self, mock_context):
        ...
        mock_context.api_client.remove_tag_from_contact.side_effect = Exception(
            "API Error"
        )
