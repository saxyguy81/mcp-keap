...
class TestApiParameterOptimizer:
    ...
    def test_optimize_contact_query_parameters_server_optimizable(self):
        ...
        assert result.optimization_strategy in [
            "highly_optimized",
    ...
    def test_analyze_filter_performance_poor(self):
        ...
        filters = [{"field": "custom_field", "operator": "CONTAINS", "value": "a"}]
...
class TestQueryExecutor:
    ...
    async def test_execute_optimized_query_direct_api(self):
        ...
        with patch.object(
            executor.optimizer, "analyze_query", return_value="bulk_retrieve"
        ):
            with patch.object(
                executor,
                "_execute_bulk_retrieve",
                return_value=(mock_contacts, 1, 0, 0),
            ):
    ...
    async def test_execute_optimized_query_hybrid_optimization(self):
        ...
        with patch.object(executor.optimizer, "analyze_query", return_value="hybrid"):
            with patch.object(
                executor.api_optimizer,
                "optimize_contact_query_parameters",
                return_value=mock_optimization,
            ):
                with patch(
                    "src.utils.contact_utils.get_custom_field_value", return_value="VIP"
                ):
                    with patch(
        ...
        with patch.object(
            ...
            with pytest.raises(Exception, match="API Error"):
                await executor.execute_optimized_query(
                    "list_contacts", filters=[], limit=50
                )
    ...
    def test_generate_cache_key(self):
        ...
        key1 = executor._generate_cache_key(
            "list_contacts", filters, 50, 0, "email", "ASC"
        )
        key2 = executor._generate_cache_key(
            "list_contacts", filters, 50, 0, "email", "ASC"
        )
        key3 = executor._generate_cache_key(
            "list_contacts", filters, 100, 0, "email", "ASC"
        )  # Different limit
    ...
    async def test_apply_client_side_filters(self):
        ...
        contacts = [
            ...
            {
                "id": 3,
                "custom_fields": [{"id": 8, "content": "VIP"}],
            },  # Different field
        ]
        ...
        with patch("src.utils.contact_utils.get_custom_field_value") as mock_get_field:
            ...
            filtered_contacts = await executor._apply_client_side_filters(
                contacts, filters
            )
