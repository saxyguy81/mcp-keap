...
class TestCacheAPIIntegration:
    ...
    async def test_cache_with_real_api_calls(self, integration_client, temp_cache_db):
        ...
        with patch("src.mcp.tools.get_cache_manager", return_value=test_cache):
            ...
            tags1 = (
                response1.get("tags", []) if isinstance(response1, dict) else response1
            )
            ...
            if not (
                hasattr(integration_client, "_is_mock") and integration_client._is_mock
            ):
    ...
    async def test_cache_invalidation_with_api(self, integration_client, temp_cache_db):
        ...
        with patch("src.mcp.tools.get_cache_manager", return_value=test_cache):
            ...
            if tags:
                ...
                assert (
                    "total_entries" in initial_stats
                    or "total_entries" in post_invalidation_stats
                )
    ...
    async def test_cache_hit_miss_ratios(self, integration_client, temp_cache_db):
        ...
        with patch("src.mcp.tools.get_cache_manager", return_value=test_cache):
            ...
            for i in range(5):
                ...
                if (
                    i == 0
                    and hasattr(integration_client, "_is_mock")
                    and integration_client._is_mock
                ):
...
class TestCacheDataConsistency:
    ...
    async def test_partial_cache_invalidation(self, integration_client, temp_cache_db):
        ...
        with patch("src.mcp.tools.get_cache_manager", return_value=test_cache):
            ...
            tag_response.get("tags", []) if isinstance(
                tag_response, dict
            ) else tag_response

            # Query contacts (different cache entries)
            contact_response = await integration_client.get_contacts(limit=5)
            contact_response.get("contacts", []) if isinstance(
                contact_response, dict
            ) else contact_response

            # Manually add some cache entries for testing
            for i in range(5):
                test_cache.set(
                    f"tag_cache_{i}", {"id": i, "name": f"Tag {i}"}, ttl=3600
                )
                test_cache.set(
                    f"contact_cache_{i}", {"id": i, "name": f"Contact {i}"}, ttl=3600
                )
...
class TestCacheWithMCPTools:
    ...
    @pytest.mark.asyncio
    async def test_cache_with_query_contacts_tool(
        self, integration_client, temp_cache_db
    ):
...
class TestCacheErrorHandling:
    ...
    async def test_cache_failure_fallback(self, integration_client, temp_cache_db):
        ...
        with patch.object(test_cache, "get", side_effect=Exception("Cache error")):
            with patch("src.mcp.tools.get_cache_manager", return_value=test_cache):
                ...
                response = await integration_client.get_tags(limit=100)
                tags = (
                    response.get("tags", []) if isinstance(response, dict) else response
                )
    ...
    @pytest.mark.asyncio
    async def test_corrupted_cache_data_handling(
        self, integration_client, temp_cache_db
    ):
