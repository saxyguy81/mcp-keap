...
async def mcp_server(event_loop):
    ...
    server_task = event_loop.create_task(server.run_async(host=MCP_HOST, port=MCP_PORT))
...
async def test_get_keap_schema(mcp_client):
    ...
    response = await mcp_client.get(
        f"http://{MCP_HOST}:{MCP_PORT}/resource/keap://schema"
    )
...
async def test_get_keap_capabilities(mcp_client):
    ...
    response = await mcp_client.get(
        f"http://{MCP_HOST}:{MCP_PORT}/resource/keap://capabilities"
    )
    assert response.status_code == 200
...
async def test_query_contacts_by_name(mcp_client):
    ...
    query = {
        "filters": [{"field": "first_name", "operator": "pattern", "value": "John*"}],
        "max_results": 10,
    }

    response = await mcp_client.post(
        f"http://{MCP_HOST}:{MCP_PORT}/query_contacts", json=query
    )
    assert response.status_code == 200
...
async def test_query_contacts_with_tag_filter(mcp_client):
    ...
    tag_query = {"max_results": 5}

    tag_response = await mcp_client.post(
        f"http://{MCP_HOST}:{MCP_PORT}/query_tags", json=tag_query
    )
    ...
    query = {
        "filters": [{"field": "tag", "operator": "=", "value": tag_id}],
        "max_results": 10,
    }
...
async def test_get_contact_details(mcp_client):
    ...
    response = await mcp_client.post(
        f"http://{MCP_HOST}:{MCP_PORT}/get_contact_details", json=details_query
    )
    assert response.status_code == 200
...
async def test_custom_field_filtering(mcp_client):
    ...
    details_query = {
        "contact_ids": query_result["contact_ids"],
        "include": {"custom_fields": {"enabled": True}},
    }
        ...
        "filters": [
            {
                "field": "custom_field",
                "operator": "=",
                "value": {"id": custom_field_id, "value": custom_field_value},
            }
        ],
        "max_results": 10,
    }
