...
class TestMaxCoverageIntegration:
    ...
    async def test_api_client_maximum_coverage(self):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            contacts_response = create_response(
                content={
                    "contacts": [
                        {
                            "id": 1,
            ...
            contact_response = create_response(
                content={
                    "id": 1,
                    "given_name": "John",
                    "family_name": "Doe",
            ...
            tag_response = create_response(
                content={"id": 10, "name": "Customer", "description": "Customer tag"}
            )
            ...
            contacts4 = await client.get_contacts(
                given_name="John", family_name="Doe", limit=50, offset=10
            )
    ...
    async def test_cache_system_maximum_coverage(self, temp_db_path):
        ...
        try:
            ...
            await cache_manager.set(
                "contacts:list", [{"id": 123}, {"id": 456}], ttl=3600
            )
        ...
        persistent_cache = PersistentCacheManager(
            db_path=temp_db_path + "_persistent", max_entries=200, max_memory_mb=20
        )
        ...
        try:
            ...
            await persistent_cache.set(
                "api:v2:users:1", {"user": 1, "version": 2}, ttl=3600
            )
            ...
            for key in contact_related_keys:
                await persistent_cache.set(
                    key, {"contact_id": 789, "data": key}, ttl=3600
                )
            ...
            with patch("src.mcp.contact_tools.get_cache_manager") as mock_get_cache:
                ...
                mock_contacts = [
                    ...
                    },
                    {
                        ...
                        "family_name": "Johnson",
                        "email_addresses": [
                            {"email": "bob@example.com", "field": "EMAIL1"}
                        ],
                        "tag_ids": [20],
                        "custom_fields": [],
                    },
    ...
    async def test_tag_tools_maximum_coverage(self):
        ...
        with patch("src.mcp.tag_tools.get_api_client") as mock_get_api:
            with patch("src.mcp.tag_tools.get_cache_manager") as mock_get_cache:
                ...
                mock_api.create_tag.return_value = {
                    "id": 100,
                    "name": "New Tag",
                ...
                for name, description, category_id in create_scenarios:
                    result = await create_tag(
                        mock_context, name, description, category_id
                    )
                    assert result["success"] is True
    ...
    async def test_utility_functions_maximum_coverage(self):
        ...
        comprehensive_contacts = [
            {
                ...
                "addresses": [
                    {"line1": "123 Main St", "locality": "Anytown", "region": "CA"}
                ],
        ...
        for contact in comprehensive_contacts:
            ...
            expected_name = " ".join(
                filter(None, [contact.get("given_name"), contact.get("family_name")])
            )
        ...
        pattern_scenarios = [("*", 3), ("John*", 1), ("*Smith", 1), ("*o*", 2)]
