...
class TestWorkingIntegration:
    ...
    async def test_filter_integration_with_name_patterns(self, sample_contacts):
        ...
        name_items = [{"name": contact["given_name"]} for contact in sample_contacts]
    ...
    @pytest.mark.asyncio
    async def test_concurrent_integration_operations(
        self, cache_manager, sample_contacts
    ):
        ...
        async def cache_operation(worker_id):
            ...
            await cache_manager.set(
                cache_key, {"worker": worker_id, "data": "test"}, ttl=3600
            )
        ...
        for i in range(5):
            tasks.extend(
                [cache_operation(i), filter_operation(i), utility_operation(i)]
            )
