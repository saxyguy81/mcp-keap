...
class TestRestoreCoverage:
        ...
        with patch("src.mcp.tools.get_api_client") as mock_get_api:
            with patch("src.mcp.tools.get_cache_manager") as mock_get_cache:
                ...
                mock_api.get_contact.side_effect = lambda contact_id: next(
                    (c for c in comprehensive_contacts if c["id"] == int(contact_id)),
                    None,
                )
                ...
                mock_cache.get.return_value = (
                    None  # Always cache miss for full execution
                )
        ...
        with patch("src.mcp.tools.get_api_client") as mock_get_api:
            with patch("src.mcp.tools.get_cache_manager") as mock_get_cache:
                ...
                new_tag = await create_tag(
                    mock_context, "Special Offer", "Special offer tag", "2"
                )
    ...
    async def test_advanced_filtering_integration(self, comprehensive_contacts):
        ...
        assert (
            len(filtered_contacts) == 2
        )  # John and Bob have 'o' in name, John has tag 10

        # Test name pattern filtering
        name_items = [
            {"name": f"{c['given_name']} {c['family_name']}"}
            for c in comprehensive_contacts
        ]
        ...
        with patch("src.mcp.tools.get_api_client") as mock_get_api:
            with patch("src.mcp.tools.get_cache_manager") as mock_get_cache:
                ...
                vip_contacts = [
                    ...
                    for c in comprehensive_contacts
                    if any(
                        cf.get("content") == "VIP" for cf in c.get("custom_fields", [])
                    )
                ]
    ...
    async def test_performance_analysis_integration(self, mock_context):
        ...
        with patch("src.mcp.tools.get_api_client") as mock_get_api:
            with patch("src.mcp.tools.get_cache_manager") as mock_get_cache:
                ...
                analysis = await analyze_query_performance(
                    mock_context, filters, "contact"
                )
    ...
    @pytest.mark.asyncio
    async def test_optimized_queries_integration(
        self, mock_context, comprehensive_contacts
    ):
