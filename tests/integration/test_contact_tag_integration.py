...
from src.mcp.contact_tools import (
    list_contacts, search_contacts_by_email, search_contacts_by_name,
    get_contact_details
)
from src.mcp.tag_tools import (
    get_tags, get_contacts_with_tag, apply_tags_to_contacts,
    remove_tags_from_contacts, create_tag, get_tag_details
)
...
class TestContactTagIntegration:
    ...
    def mock_api_client(self):
        ...
        client.get_tag.side_effect = lambda tag_id: next(
            (tag for tag in mock_tags if tag["id"] == int(tag_id)),
            None
        )
        ...
        client.create_tag.return_value = {
            "id": 60, "name": "New Tag", "description": "Newly created tag"
        }
    ...
    async def test_contact_listing_with_caching(self, mock_context, mock_api_client):
        ...
        with patch('src.mcp.contact_tools.get_api_client', return_value=mock_api_client), \
             patch('src.mcp.contact_tools.get_cache_manager', return_value=mock_context.cache_manager):
    ...
    @pytest.mark.asyncio
    async def test_contact_search_by_email_integration(self, mock_context, mock_api_client):
    ...
    @pytest.mark.asyncio
    async def test_contact_search_by_name_integration(self, mock_context, mock_api_client):
    ...
    async def test_tag_listing_with_filtering(self, mock_context, mock_api_client):
        ...
        with patch('src.mcp.tag_tools.get_api_client', return_value=mock_api_client), \
             patch('src.mcp.tag_tools.get_cache_manager', return_value=mock_context.cache_manager):
    ...
    async def test_contacts_with_tag_integration(self, mock_context, mock_api_client):
        ...
        with patch('src.mcp.tag_tools.get_api_client', return_value=mock_api_client), \
            ...
            contacts_with_tag = await get_contacts_with_tag(mock_context, "10", limit=50)
    ...
    @pytest.mark.asyncio
    async def test_tag_application_and_removal_workflow(self, mock_context, mock_api_client):
        ...
        with patch('src.mcp.tag_tools.get_api_client', return_value=mock_api_client), \
            ...
            remove_result = await remove_tags_from_contacts(mock_context, tag_ids, contact_ids)
    ...
    @pytest.mark.asyncio
    async def test_contact_and_tag_details_integration(self, mock_context, mock_api_client):
    ...
    @pytest.mark.asyncio
    async def test_cache_invalidation_on_tag_operations(self, mock_context, mock_api_client):
        ...
        with patch('src.mcp.tag_tools.get_api_client', return_value=mock_api_client), \
            ...
            # Cache some tag data
            cache_key = "tags:all"
            await mock_context.cache_manager.set(cache_key, [{"id": 10, "name": "Customer"}], ttl=3600)
    ...
    async def test_bulk_contact_tag_operations(self, mock_context, mock_api_client):
        ...
        with patch('src.mcp.tag_tools.get_api_client', return_value=mock_api_client), \
            ...
            bulk_apply_result = await apply_tags_to_contacts(mock_context, tag_ids, contact_ids)
    ...
    @pytest.mark.asyncio
    async def test_concurrent_contact_tag_operations(self, mock_context, mock_api_client):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_in_contact_tag_operations(self, mock_context, mock_api_client):
