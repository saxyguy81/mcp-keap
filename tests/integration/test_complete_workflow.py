...
class TestKeapMCPIntegration:
    ...
    async def mock_api_service(self):
        ...
        with patch("src.api.client.KeapApiService") as mock:
            ...
            service.get_tags.return_value = {
                "tags": [{"id": 1, "name": "VIP"}, {"id": 2, "name": "Customer"}]
            }
    ...
    async def test_contact_workflow(
        self, integration_client, cache_manager, mock_environment
    ):
        ...
        contacts = (
            contacts_response.get("contacts", [])
    ...
    @pytest.mark.asyncio
    async def test_custom_field_workflow(
        self, integration_client, cache_manager, mock_environment
    ):
...
class TestErrorRecoveryIntegration:
    ...
    async def test_cache_failure_recovery(self, mock_environment):
        ...
        with patch(
            "src.mcp.contact_tools.get_api_client", return_value=mock_api_service
        ):
            with patch(
