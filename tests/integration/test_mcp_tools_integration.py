...
from src.mcp.tools import (
    get_api_client, get_cache_manager, set_custom_field_values,
...
class TestMCPToolsIntegration:
    ...
    @pytest.mark.asyncio
    async def test_contact_tools_integration(self, context_with_mocks, mock_environment):
        """Test contact tools integration."""
        context = context_with_mocks
        
        # Test list contacts
        with patch('src.mcp.contact_tools.get_api_client', return_value=context.api_client):
            with patch('src.mcp.contact_tools.get_cache_manager', return_value=context.cache_manager):
    ...
    async def test_tag_tools_integration(self, context_with_mocks, mock_environment):
        ...
        with patch('src.mcp.tag_tools.get_api_client', return_value=context.api_client):
            with patch('src.mcp.tag_tools.get_cache_manager', return_value=context.cache_manager):
    ...
    @pytest.mark.asyncio
    async def test_custom_field_tools_integration(self, context_with_mocks, mock_environment):
        ...
        assert context.cache_manager.invalidate_pattern.call_count == 4  # 2 patterns × 2 contacts
    
    @pytest.mark.asyncio
    async def test_custom_field_individual_values(self, context_with_mocks, mock_environment):
    ...
    @pytest.mark.asyncio
    async def test_utility_functions_integration(self, context_with_mocks, mock_environment):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_integration(self, mock_cache_manager, mock_environment):
