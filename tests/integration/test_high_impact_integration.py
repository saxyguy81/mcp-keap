...
class TestHighImpactIntegration:
    ...
    @pytest.mark.asyncio
    async def test_mcp_tools_with_mocked_dependencies(
        ...
        mock_api_client.get_contact.side_effect = (
            lambda contact_id: sample_contacts[0]
            if contact_id == "1"
            else sample_contacts[1]
        )
        mock_api_client.get_tags.return_value = {"tags": sample_tags}
        mock_api_client.get_tag.side_effect = (
            lambda tag_id: sample_tags[0] if tag_id == "10" else sample_tags[1]
        )
        ...
        with (
            patch.object(tools, "get_api_client", return_value=mock_api_client),
            ...
            email_results = await tools.search_contacts_by_email(
    ...
    @pytest.mark.asyncio
    async def test_contact_tools_comprehensive_integration(
        ...
        with (
            patch.object(contact_tools, "get_api_client", return_value=mock_api_client),
            patch.object(
                contact_tools, "get_cache_manager", return_value=mock_cache_manager
            ),
        ):
            # Test list_contacts with various parameters
            contacts = await contact_tools.list_contacts(
                mock_context, limit=50, offset=0
            )
            assert len(contacts) == 2
            ...
            # Test search_contacts_by_email
            email_search = await contact_tools.search_contacts_by_email(
                mock_context, "john@example.com"
            )
            assert len(email_search) >= 1

            # Test search_contacts_by_name
            name_search = await contact_tools.search_contacts_by_name(
                mock_context, "Jane"
            )
    ...
    async def test_tag_tools_comprehensive_integration(self, mock_context, sample_tags):
        ...
        with (
            patch.object(tag_tools, "get_api_client", return_value=mock_api_client),
            patch.object(
                tag_tools, "get_cache_manager", return_value=mock_cache_manager
            ),
        ):
            # Test get_tags
    ...
    async def test_api_client_comprehensive_coverage(self):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            mock_client.get.side_effect = lambda url, **kwargs: mock_response(
                url, "GET", **kwargs
            )
            mock_client.post.side_effect = lambda url, **kwargs: mock_response(
                url, "POST", **kwargs
            )
            mock_client.put.side_effect = lambda url, **kwargs: mock_response(
                url, "PUT", **kwargs
            )
    ...
    async def test_concurrent_operations_integration(
        ...
        async def contact_operations():
            with (
                patch.object(tools, "get_api_client", return_value=mock_api_client),
                patch.object(
                    tools, "get_cache_manager", return_value=mock_cache_manager
                ),
            ):
                contacts = await tools.list_contacts(mock_context)
                return len(contacts)
    ...
    async def test_comprehensive_workflow_integration(
        ...
        with (
            ...
            # 4. Search operations
            email_search = await tools.search_contacts_by_email(
                mock_context, "john@example.com"
            )
