...
class TestMCPServerCompleteIntegration:
    ...
    def mock_api_client(self):
        ...
        mock_contacts = [
            {
                ...
                "tag_ids": [10, 20, 30],
                "custom_fields": [{"id": 7, "content": "VIP"}, {"id": 8, "content": "Premium"}],
        ...
        client.get_contact.side_effect = lambda contact_id: next(
            (contact for contact in mock_contacts if contact["id"] == int(contact_id)), None
        )
        ...
        client.create_tag.return_value = {"id": 60, "name": "New Tag", "description": "New tag"}
    ...
    async def test_complete_contact_workflow(self, mock_context, mock_api_client):
        ...
        with patch('src.mcp.tools.get_api_client', return_value=mock_api_client), \
             patch('src.mcp.tools.get_cache_manager', return_value=mock_context.cache_manager):
    ...
    async def test_optimized_query_workflow(self, mock_context, mock_api_client):
        ...
        with patch('src.mcp.tools.get_api_client', return_value=mock_api_client), \
            ...
            with patch('src.mcp.optimization.api_optimization.ApiParameterOptimizer') as mock_api_opt, \
                ...
                assert performance_analysis["query_analysis"]["optimization_score"] == 0.9
    ...
    @pytest.mark.asyncio
    async def test_diagnostics_and_monitoring_workflow(self, mock_context, mock_api_client):
    ...
    @pytest.mark.asyncio
    async def test_data_consistency_across_operations(self, mock_context, mock_api_client):
