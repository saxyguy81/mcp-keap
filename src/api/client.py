...
class KeapApiService:
    ...
    async def _wait_for_rate_limit(self):
        ...
        if self._should_rate_limit():
            if self.daily_request_count >= self.daily_request_limit:
                ...
                logger.warning(
                    f"Daily limit reached, waiting {wait_time / 3600:.1f} hours"
                )
    ...
    def _update_diagnostics(
        ...
        if success:
            ...
            if error:
                self.diagnostics["error_counts"][error] = (
                    self.diagnostics["error_counts"].get(error, 0) + 1
                )
        ...
        self.diagnostics["endpoints_called"][endpoint] = (
            self.diagnostics["endpoints_called"].get(endpoint, 0) + 1
        )
    ...
    def get_diagnostics(self) -> Dict[str, Any]:
        ...
        return {
            **self.diagnostics,
            ...
            "daily_requests_remaining": self.daily_request_limit
            - self.daily_request_count,
            "uptime_hours": (time.time() - self.request_start_of_day) / 3600,
            "requests_per_hour": self.diagnostics["total_requests"]
            / max((time.time() - self.request_start_of_day) / 3600, 0.1),
        }
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Rate limited, waiting {retry_after} seconds before retry {attempt + 1}"
                    )
                ...
                wait_time = self.retry_delay * (2**attempt)
                logger.warning(
                    f"Timeout error, retrying in {wait_time}s (attempt {attempt + 1})"
                )
                if attempt < self.max_retries:
                    await asyncio.sleep(wait_time)
                    continue
                ...
                wait_time = self.retry_delay * (2**attempt)
                logger.warning(
                    f"Network error: {e}, retrying in {wait_time}s (attempt {attempt + 1})"
                )
                if attempt < self.max_retries:
                    await asyncio.sleep(wait_time)
                    continue
                ...
                if 400 <= e.response.status_code < 500:
                    ...
                    wait_time = self.retry_delay * (2**attempt)
                    logger.warning(
                        f"HTTP error {e.response.status_code}, retrying in {wait_time}s (attempt {attempt + 1})"
                    )
                    if attempt < self.max_retries:
                        await asyncio.sleep(wait_time)
                        continue
    ...
    async def get_contacts(
        self, limit: int = 200, offset: int = 0, **filters
    ) -> Dict[str, Any]:
    ...
    async def update_contact_custom_field(
        ...
        try:
            ...
            custom_field_data = {
                "custom_fields": [{"id": int(field_id), "content": str(value)}]
            }
            ...
            logger.error(
                f"Error updating custom field {field_id} for contact {contact_id}: {e}"
            )
    ...
    async def get_tags(self, limit: int = 1000, offset: int = 0) -> Dict[str, Any]:
        ...
        if (
            self._tag_cache
            and current_time - self._tag_cache_timestamp < self._tag_cache_ttl
        ):
    ...
    async def get_contacts_by_tag(
        self, tag_id: str, limit: int = 200
    ) -> List[Dict[str, Any]]:
        """Get all contacts with a specific tag"""
        return await self.get_paginated(
            "/contacts", params={"tag_id": tag_id}, limit=limit
        )
