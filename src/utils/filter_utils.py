...
def filter_by_name_pattern(
    items: List[Dict[str, Any]], pattern: str
) -> List[Dict[str, Any]]:
    ...
    for i, filter_condition in enumerate(filters):
        ...
        if "operator" in filter_condition and "conditions" in filter_condition:
            ...
            if operator not in ["AND", "OR", "NOT"]:
                raise ValueError(
                    f"Filter {i} has invalid logical operator '{operator}'"
                )
        ...
        if "value" not in filter_condition and filter_condition.get("operator") not in [
            "IS_NULL",
            "IS_NOT_NULL",
        ]:
        ...
        if operator not in valid_operators:
            raise ValueError(
                f"Filter {i} has invalid operator '{operator}'. Valid operators: {valid_operators}"
            )
        ...
        if operator in ["IN", "NOT_IN"] and not isinstance(value, list):
            raise ValueError(
                f"Filter {i} with operator '{operator}' requires a list value"
            )

        if operator == "BETWEEN" and (not isinstance(value, list) or len(value) != 2):
            raise ValueError(
                f"Filter {i} with operator 'BETWEEN' requires a list of exactly 2 values"
            )
...
def evaluate_filter_condition(item: Dict[str, Any], condition: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if item_value is None:
            return operator in ["IS_NULL", "NOT_EQUALS"] or (
                operator == "EQUALS" and filter_value is None
            )
...
def apply_complex_filters(
    items: List[Dict[str, Any]], filters: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
...
def optimize_filters_for_api(
    ...
    for filter_condition in filters:
        ...
        normalized_operator = (
            operator.upper() if isinstance(operator, str) else operator
        )
        if operator == "=":
        ...
        if field in server_side_fields and normalized_operator in [
            "EQUALS",
            "CONTAINS",
        ]:
