...
class ModifyTagsRequest(BaseModel):
    ...
    tags_to_remove: Optional[List[int]] = Field(default=[], description="Tag IDs to remove")
    use_v2_api: bool = Field(default=True, description="Use V2 API for bulk operations")
    batch_size: Optional[int] = Field(default=50, ge=1, le=100, description="Batch size")
...
class Tag(BaseModel):
    ...
    category: Optional[Dict[str, Any]] = Field(
        default=None, description="Tag category"
    )
...
class QueryMetadata(BaseModel):
    ...
    filters_applied: int = Field(
        default=0, description="Number of filters applied"
    )
