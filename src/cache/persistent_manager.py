...
class PersistentCacheManager:
    ...
    def __init__(self, db_path: str = "keap_cache.db", max_entries: int = 10000, max_memory_mb: int = 100):
    ...
    def _init_database(self) -> None:
        ...
        with self._get_connection() as conn:
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_contact_mapping_id ON contact_cache_mapping(contact_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_tag_mapping_id ON tag_cache_mapping(tag_id)')
                ...
                with self._get_connection() as conn:
                    ...
                    conn.execute(
                        '''INSERT OR REPLACE INTO cache 
                           ...
                           VALUES (?, ?, ?, ?, ?, ?)''',
                        (key, serialized_value, expires_at, current_time, value_size, current_time)
                    )
    ...
    def _ensure_space(self, required_bytes: int) -> None:
        ...
        try:
            with self._get_connection() as conn:
                ...
                if count >= self.max_entries or (total_size + required_bytes) > self.max_memory_bytes:
                    # Remove least recently used entries
                    entries_to_remove = max(1, int(self.max_entries * 0.1))  # Remove 10% of max entries
                    cursor = conn.execute(
                    ...
                    logger.debug(f"Removed {len(keys_to_remove)} cache entries to make space")
        ...
        with self._lock:
            try:
                with self._get_connection() as conn:
                    ...
                    cursor = conn.execute(
                        f'SELECT DISTINCT cache_key FROM contact_cache_mapping WHERE contact_id IN ({placeholders})',
                        [int(cid) for cid in contact_ids if isinstance(cid, (int, str))]
                    )
                    ...
                    logger.info(f"Invalidated {len(keys_to_invalidate)} cache entries for {len(contact_ids)} contacts")
        ...
        with self._lock:
            try:
                with self._get_connection() as conn:
                    ...
                    cursor = conn.execute(
                        f'SELECT DISTINCT cache_key FROM tag_cache_mapping WHERE tag_id IN ({placeholders})',
                    ...
                    logger.info(f"Invalidated {len(keys_to_invalidate)} cache entries for {len(tag_ids)} tags")
        ...
        try:
            with self._get_connection() as conn:
                ...
                return {
                    ...
                    "memory_usage_percent": round(((total_size or 0) / self.max_memory_bytes) * 100, 2),
                    "entry_usage_percent": round(((count or 0) / self.max_entries) * 100, 2)
                }
        except sqlite3.Error as e:
