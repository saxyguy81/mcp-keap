...
class CacheManager:
    ...
    def __init__(self, db_path: str = "keap_cache.db", max_entries: int = 10000, max_memory_mb: int = 100):
        ...
        """
        self._persistent_cache = PersistentCacheManager(db_path, max_entries, max_memory_mb)
        logger.info(f"Initialized persistent cache at {db_path} with {max_entries} max entries and {max_memory_mb}MB memory limit")
