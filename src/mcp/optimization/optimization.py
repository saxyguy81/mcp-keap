...
class QueryOptimizer:
    ...
    def analyze_query(self, filters: List[Dict[str, Any]], 
                     limit: int = 200, 
                     hints: Optional[OptimizationHint] = None) -> QueryStrategy:
        """
        ...
        tag_filters = [f for f in filters if f.get('field') in ['tags', 'tag_id', 'tag_ids']]
    ...
    def _count_server_optimizable_filters(self, filters: List[Dict[str, Any]]) -> int:
        ...
        for filter_condition in filters:
            if (filter_condition.get('field') in server_fields and 
                filter_condition.get('operator') in server_operators):
                count += 1
    ...
    def track_performance(self, query_key: str, metrics: QueryMetrics):
        ...
        if len(self.performance_history[query_key]) > 20:
            self.performance_history[query_key] = self.performance_history[query_key][-20:]
    ...
    def get_performance_summary(self, query_key: Optional[str] = None) -> Dict[str, Any]:
...
class QueryExecutor:
    ...
    async def execute_optimized_query(self, 
                                    ...
                                    limit: int = 200,
                                    offset: int = 0,
                                    **kwargs) -> Tuple[List[Dict[str, Any]], QueryMetrics]:
        """
        ...
        logger.debug(f"Selected strategy: {strategy} for query with {len(filters)} filters")
        
        # Execute based on strategy
        if strategy == QueryStrategy.SERVER_OPTIMIZED:
            results, api_calls, server_filters, client_filters = await self._execute_server_optimized(
                query_type, filters, limit, offset, **kwargs
            )
        elif strategy == QueryStrategy.TAG_OPTIMIZED:
            results, api_calls, server_filters, client_filters = await self._execute_tag_optimized(
                query_type, filters, limit, offset, **kwargs
            )
        elif strategy == QueryStrategy.HYBRID:
            results, api_calls, server_filters, client_filters = await self._execute_hybrid(
                query_type, filters, limit, offset, **kwargs
            )
        else:
            # Default to bulk retrieve
            results, api_calls, server_filters, client_filters = await self._execute_bulk_retrieve(
                query_type, filters, limit, offset, **kwargs
            )
    ...
    async def _execute_server_optimized(self, query_type: str, filters: List[Dict[str, Any]], 
                                      limit: int, offset: int, **kwargs) -> Tuple[List[Dict[str, Any]], int, int, int]:
        """Execute with maximum server-side optimization."""
        from src.utils.filter_utils import optimize_filters_for_api, apply_complex_filters
    ...
    async def _execute_tag_optimized(self, query_type: str, filters: List[Dict[str, Any]], 
                                   limit: int, offset: int, **kwargs) -> Tuple[List[Dict[str, Any]], int, int, int]:
    ...
    def _generate_cache_key(self, query_type: str, filters: List[Dict[str, Any]], 
                           limit: int, offset: int, order_by: str = None, 
                           order_direction: str = "ASC") -> str:
    ...
    async def _apply_client_side_filters(self, contacts: List[Dict[str, Any]], 
