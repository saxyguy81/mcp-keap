...
async def remove_tags_from_contacts(
    ...
    from src.mcp.tag_tools import remove_tags_from_contacts as _remove_tags_from_contacts
    ...
    try:
        ...
        for list_item in lists:
            ...
            if not isinstance(ids, list):
                return {
                    "success": False,
                    "error": f"Field '{id_field}' must be a list"
                }
        ...
        logger.info(f"Intersected {len(lists)} lists, found {len(result_ids)} common IDs")
        return {
            "success": True,
    ...
    try:
        ...
        for contact in all_contacts:
            ...
            if custom_field_value is not None:
                ...
                elif operator == "contains" and str(field_value).lower() in str(custom_field_value).lower():
                    matching_contacts.append(contact)
                elif operator == "starts_with" and str(custom_field_value).lower().startswith(str(field_value).lower()):
                    matching_contacts.append(contact)
        
        # Process include fields
        if include:
            from src.utils.contact_utils import process_contact_include_fields
            matching_contacts = [process_contact_include_fields(contact, include) for contact in matching_contacts]
        
        # Format contacts
        from src.utils.contact_utils import format_contact_data
        formatted_contacts = [format_contact_data(contact) for contact in matching_contacts]
        
        # Cache result
        await cache_manager.set(cache_key, formatted_contacts, ttl=1800)
        
        logger.info(f"Found {len(formatted_contacts)} contacts with custom field {field_id}={field_value}")
...
async def query_contacts_optimized(
    ...
    try:
        ...
        if enable_optimization:
            ...
            logger.info(f"Optimized query returned {len(contacts)} contacts using {metrics.strategy_used} strategy")
            return result
...
async def analyze_query_performance(
    ...
    try:
        ...
        performance_analysis = api_optimizer.analyze_filter_performance(filters, query_type)
        ...
        result = {
            ...
            "filter_breakdown": {
                ...
                "server_optimizable_count": len(optimization_result.server_side_filters),
                "client_only_count": len(optimization_result.client_side_filters)
            },
            "optimization_suggestions": _generate_optimization_suggestions(optimization_result, filters),
            "field_capabilities": field_info
        }
        
        logger.info(f"Analyzed query with {len(filters)} filters - {performance_analysis['performance_rating']} performance rating")
        return result
...
def _generate_optimization_suggestions(optimization_result, filters: List[Dict[str, Any]]) -> List[str]:
    """Generate optimization suggestions based on analysis."""
    suggestions = []
    
    if optimization_result.optimization_score < 0.5:
        suggestions.append("Consider restructuring filters to use server-optimizable fields like 'email', 'given_name', 'family_name'")
    
    if len(optimization_result.client_side_filters) > len(optimization_result.server_side_filters):
        suggestions.append("Many filters require client-side processing - consider caching results for repeated queries")
    
    # Check for logical groups
    logical_groups = [f for f in filters if 'operator' in f and 'conditions' in f]
    if logical_groups:
        suggestions.append("Complex logical conditions detected - ensure you're using the optimized query endpoint for best performance")
    ...
    for filter_condition in optimization_result.client_side_filters:
        if 'field' in filter_condition:
            ...
            if field not in ['tags', 'custom_fields']:  # These are expected to be client-side
                unsupported_fields.append(field)
    
    if unsupported_fields:
        suggestions.append(f"Fields {unsupported_fields} cannot be optimized server-side with current operators")
    
    if optimization_result.optimization_score >= 0.8:
        suggestions.append("Excellent optimization! This query should perform very well")
...
async def modify_tags(
    ...
    try:
        if action == "add":
            ...
            for tag_id in tag_ids:
                result = await ctx.api_client.remove_tag_from_contacts(tag_id, contact_ids)
                if not result.get("success", False):
...
async def get_api_diagnostics(
    ...
    try:
        ...
        performance_metrics = {
            "success_rate": (
                api_diagnostics["successful_requests"] / max(api_diagnostics["total_requests"], 1) * 100
            ),
            "retry_rate": (
                api_diagnostics["retried_requests"] / max(api_diagnostics["total_requests"], 1) * 100
            ),
            "rate_limit_hit_rate": (
                api_diagnostics["rate_limited_requests"] / max(api_diagnostics["total_requests"], 1) * 100
            ),
            "cache_hit_rate": (
                api_diagnostics["cache_hits"] / max(api_diagnostics["cache_hits"] + api_diagnostics["cache_misses"], 1) * 100
            )
        }
...
def _generate_performance_recommendations(api_diagnostics: Dict, performance_metrics: Dict) -> List[str]:
    """Generate performance recommendations based on diagnostics."""
    recommendations = []
    
    if performance_metrics["success_rate"] < 95:
        recommendations.append("Success rate is below 95% - check error patterns and consider API key/permissions")
    
    if performance_metrics["retry_rate"] > 10:
        recommendations.append("High retry rate detected - API may be unstable or rate limits too aggressive")
    
    if performance_metrics["rate_limit_hit_rate"] > 5:
        recommendations.append("Frequent rate limiting - consider reducing request frequency or implementing better backoff")
    ...
    if api_diagnostics["average_response_time"] > 2.0:
        recommendations.append("High average response time - check network conditions and API performance")
    
    if api_diagnostics["requests_per_hour"] > 5000:
        recommendations.append("High request rate - monitor daily limits and consider optimization")
...
async def set_custom_field_values(
    ...
    try:
        ...
        if successful_updates:
            response["successful_contacts"] = [u["contact_id"] for u in successful_updates]
        
        if failed_updates:
            response["failed_contacts"] = failed_updates
            response["error_summary"] = f"{total_failed} out of {total_requested} updates failed"
        
        if total_failed == 0:
            response["message"] = f"Successfully updated custom field {field_id} for {total_successful} contacts"
        elif total_successful > 0:
            response["message"] = f"Partially successful: {total_successful} updated, {total_failed} failed"
        else:
            response["message"] = f"All {total_failed} updates failed"
        
        logger.info(f"Custom field update: {total_successful}/{total_requested} successful for field {field_id}")
        return response
            ...
            "oneOf": [
                {
                    ...
                    "not": {"anyOf": [{"required": ["contact_ids"]}, {"required": ["common_value"]}]}
                },
                {
