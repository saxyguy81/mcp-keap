...
async def validate_contacts_endpoint():
    ...
    try:
        ...
        if contacts:
            ...
            logger.info(
                f"Sample contact structure: {json.dumps(sample_contact, indent=2)}"
            )

            # Validate required fields
            required_fields = ["id", "given_name", "family_name", "email_addresses"]
            missing_fields = [
                field for field in required_fields if field not in sample_contact
            ]

            if missing_fields:
                logger.warning(
                    f"Missing required fields in contact response: {missing_fields}"
                )
            else:
            ...
            if "email_addresses" in sample_contact:
                ...
                logger.info(
                    f"Email addresses structure: {json.dumps(email_addresses, indent=2)}"
                )
        ...
        logger.info(
            f"Retrieved {len(date_filtered_contacts)} contacts with date filters"
        )

        # Test with optional fields
        logger.info("Testing optional fields...")
        params = {"page_size": 10, "optional_properties": "tag_ids,custom_fields"}
        ...
        if optional_contacts:
            ...
            logger.info(
                f"Has tag_ids: {has_tag_ids}, Has custom_fields: {has_custom_fields}"
            )

            if has_tag_ids:
                logger.info(
                    f"Tag IDs structure: {json.dumps(sample_contact['tag_ids'], indent=2)}"
                )

            if has_custom_fields:
                logger.info(
                    f"Custom fields structure: {json.dumps(sample_contact['custom_fields'], indent=2)}"
                )
...
async def validate_tags_endpoint():
    ...
    try:
        ...
        if all_tags:
            ...
            missing_fields = [
                field for field in required_fields if field not in sample_tag
            ]

            if missing_fields:
                logger.warning(
                    f"Missing required fields in tag response: {missing_fields}"
                )
            else:
            ...
            logger.info(
                f"Retrieved {len(tagged_contacts)} contacts with tag ID {tag_id}"
            )
            ...
            if tagged_contacts:
                ...
                logger.info(
                    f"Sample tagged contact structure: {json.dumps(sample_contact, indent=2)}"
                )
    ...
    try:
        ...
        for contact_id in contact_ids:
            if contact_id in tagged_contact_ids:
                ...
                logger.warning(
                    f"Failed to verify tag application for contact {contact_id}"
                )
...
async def validate_custom_fields():
    ...
    try:
        ...
        params = {"page_size": 10, "optional_properties": "custom_fields"}
        ...
        contacts_with_custom_fields = [
            c for c in contacts if "custom_fields" in c and c["custom_fields"]
        ]
        ...
        logger.info(
            f"Found {len(contacts_with_custom_fields)} contacts with custom fields"
        )

        # Check the structure of custom fields
        for i, contact in enumerate(
            contacts_with_custom_fields[:3]
        ):  # Limit to 3 contacts
            ...
            if isinstance(custom_fields, list):
                ...
                for cf in custom_fields:
                    logger.info(
                        f"  Field ID: {cf.get('id')}, Name: {cf.get('name')}, Value: {cf.get('content')}"
                    )
                ...
                for field_id, field_data in custom_fields.items():
                    logger.info(
                        f"  Field ID: {field_id}, Name: {field_data.get('name')}, Value: {field_data.get('content')}"
                    )

            else:
                logger.warning(
                    f"Unexpected custom fields format: {type(custom_fields)}"
                )
    ...
    try:
        ...
        for i in range(10):
            ...
            try:
                ...
                logger.info(
                    f"Request {i + 1}: Got {len(contacts)} contacts in {elapsed:.2f}s"
                )
            except Exception as e:
        ...
        logger.info(
            f"Rate limit resets at: {datetime.fromtimestamp(client.rate_limit_reset)}"
        )
...
async def validate_contact_operations():
    ...
    try:
        ...
        logger.info(
            f"Retrieved contact details: {json.dumps(contact_details, indent=2)}"
        )
        ...
        if contact_details.get("id") == contact_id:
            ...
            logger.warning(
                f"Contact ID mismatch: Expected {contact_id}, got {contact_details.get('id')}"
            )
        ...
        if additional_fields:
            logger.info(
                f"Additional fields in single contact fetch: {additional_fields}"
            )
...
async def validate_error_handling():
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Expected error for invalid contact ID, but got result: {contact}"
            )
        except Exception as e:
        ...
        try:
            ...
            logger.warning(
                "Expected error for invalid parameters, but request succeeded"
            )
        except Exception as e:
        ...
        try:
            params = {"since": "not-a-date", "page_size": 10}
            await client.query_contacts(params)
            logger.warning(
                "Expected error for invalid date format, but request succeeded"
            )
        except Exception as e:
    ...
    try:
        ...
        for size in page_sizes:
            ...
            try:
                ...
                logger.info(
                    f"Retrieved {len(contacts)} contacts with page_size={size} in {elapsed:.2f}s"
                )
                ...
                if len(contacts) == size:
                    ...
                    logger.info(
                        f"Got fewer contacts ({len(contacts)}) than requested ({size})"
                    )
                else:
                    logger.warning(
                        f"Got more contacts ({len(contacts)}) than requested ({size})"
                    )
        ...
        while len(all_contacts) < max_contacts:
            ...
            logger.info(
                f"Retrieved page {page} with {len(contacts)} contacts, total: {len(all_contacts)}"
            )
        ...
        elapsed = (datetime.now() - start_time).total_seconds()
        logger.info(
            f"Retrieved a total of {len(all_contacts)} contacts in {elapsed:.2f}s"
        )
...
async def validate_search_capabilities():
    ...
    try:
        ...
        for domain in domains:
            params = {"page_size": 10, "email": f"*@{domain}"}

            try:
                contacts = await client.query_contacts(params)
                logger.info(
                    f"Found {len(contacts)} contacts with email domain {domain}"
                )
                ...
                if contacts:
                    ...
                    if "email_addresses" in sample_contact:
                        ...
                        if isinstance(email_addresses, list) and email_addresses:
                            ...
                            if domain in email:
                                logger.info(
                                    f"Verified email domain search worked: {email}"
                                )
                            else:
                                logger.warning(
                                    f"Email domain search may not work correctly: {email}"
                                )
        ...
        for name in names:
            params = {"page_size": 10, "given_name": name}
            ...
            try:
                ...
                if contacts:
                    ...
                    if sample_contact.get("given_name") == name:
                        logger.info(
                            f"Verified name search worked: {sample_contact.get('given_name')}"
                        )
                    else:
                        logger.warning(
                            f"Name search may not work correctly: {sample_contact.get('given_name')}"
                        )
        ...
        params = {"page_size": 10, "since": thirty_days_ago, "until": today}
