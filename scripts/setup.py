...
def setup_git_hooks():
    ...
    try:
        ...
        result = run_command([sys.executable, "-m", "pip", "show", "pre-commit"], 
                           "Checking pre-commit installation", check=False)
...
def setup_production():
    ...
    if not prod_env.exists():
        ...
        content = content.replace("ENABLE_OPTIMIZATIONS=true", "ENABLE_OPTIMIZATIONS=true")
...
def main():
    ...
    parser.add_argument("command", choices=[
        "install", "dev", "test", "check", "docker", "production", "keygen", "all"
