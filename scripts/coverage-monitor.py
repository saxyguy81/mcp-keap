...
def analyze_coverage():
    ...
    for module, data in sorted(module_coverage.items()):
        if data["total"] > 0:
            ...
            print(f"  {module:<20} {module_percent:>6.1f}% ({data['covered']}/{data['total']})")
    ...
    if high_coverage_files:
        for file_path, coverage in sorted(high_coverage_files, key=lambda x: x[1], reverse=True):
            print(f"  {file_path:<40} {coverage:>6.1f}%")
    else:
    ...
    for file_path, file_data in files.items():
        if file_path.startswith("src/"):
            ...
            if lines > 50 and coverage < 60:
                recommendations.append(f"📋 {file_path}: {coverage:.1f}% coverage, {lines} lines - High impact target")
            
            # Service files should have high coverage
            elif "service" in file_path and coverage < 80:
                recommendations.append(f"⚙️  {file_path}: {coverage:.1f}% coverage - Service layer priority")
            
            # Utility files should be fully tested
            elif "utils" in file_path and coverage < 90:
                recommendations.append(f"🔧 {file_path}: {coverage:.1f}% coverage - Utility functions need complete testing")
